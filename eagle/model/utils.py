...
def initialize_tree(input_ids, model, pixel_values, past_key_values, logits_processor):
    ...
    filtered_input_ids, filtered_hidden_states = remove_image_token_except_last(input_ids, model.base_model.config.image_token_index, hidden_states)
...
def update_inference_inputs(
    ...
    filtered_input_ids = remove_image_token_except_last(input_ids, model.base_model.config.image_token_index)
    filtered_input_ids = filtered_input_ids.to(ea_layer_device)
    filtered_input_ids = torch.cat((filtered_input_ids, token.to(filtered_input_ids.device)), dim=1)
