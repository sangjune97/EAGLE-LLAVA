...
def ge(data):
    ...
    input_ids, loss_mask, hidden_state_big = remove_image_token(input_ids, 32000, loss_mask, hidden_state_big)
    del outs_big
    
    # 캐시 정리 (가비지 컬렉션 + CUDA 캐시 해제)
    gc.collect()
    torch.cuda.empty_cache()
    td={"input_ids":input_ids.cpu()[0],"image":data["image"],"hidden_state":hidden_state_big[0].cpu(),"loss_mask":loss_mask.cpu()[0]}
